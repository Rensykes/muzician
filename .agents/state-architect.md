...
| `fretboardChordCommittedProvider` | `StateProvider<bool>` | Whether a fretboard voicing is currently committed |
| `pianoChordCommittedProvider` | `StateProvider<bool>` | Whether a piano voicing is currently committed |
| `pianoPendingChordProvider` | `StateProvider<...?>` | Temp chord being previewed (piano) |
| `pianoPendingScaleProvider` | `StateProvider<...?>` | Temp scale being previewed (piano) |
| `pendingChordProvider` (fretboard) | `StateProvider<({String root, String quality})?>` | Temp chord for fretboard detection panel |
| `pendingScaleProvider` (fretboard) | `StateProvider<({String root, String scaleName})?>` | Temp scale for fretboard detection panel |
| `pianoRollPendingScaleProvider` | `StateProvider<...?>` | Temp scale being previewed (piano roll). **No `pianoRollPendingChordProvider`** today — add only if a chord-preview UX appears in piano roll. |
...
- New theory inputs to the model → [music-theory](./music-theory.md)
- New snapshot fields → [save-system](./save-system.md) (mirror the field in `toJson`/`fromJson` and every `switch (snapshot)`)
- Provider wiring that affects painters or gesture state → [instrument-renderer](./instrument-renderer.md)
