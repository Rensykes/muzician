...
**Review-only agent.** Do not edit, create, or delete files. Use read and search tools only. Your output is a written report; a developer applies the changes.
...
| `SaveBrowserPanel` | `lib/ui/save_browser_panel.dart` | Swipe-to-delete discoverability, modal height on small screens, nested folder breadcrumbs |
| `FretboardSavePanel` / `PianoSavePanel` | `lib/features/{fretboard,piano}/*_save_panel.dart` | Wraps `SaveBrowserPanel`; review the per-instrument framing |
| `PianoRollSaveStackLoader` | `lib/features/piano_roll/piano_roll_save_stack_loader.dart` | Folder picker that loads saves *into* the piano roll — check exact / pitch-class mode toggle clarity |
| `AppInfoPanel` | `lib/ui/core/app_info_panel.dart` | Gesture/feature help sheet — primary discoverability surface for long-press / pinch / swipe gestures |
...
- Long-press to delete in piano roll — is this communicated anywhere? (`AppInfoPanel` is the canonical surface.)
- Pinch-to-zoom in piano roll — is this discoverable on first use?
- Swipe-to-delete in `SaveBrowserPanel` — is there an empty-state hint?
- Loading a fretboard/piano save into the piano roll via `PianoRollSaveStackLoader` — is the "exact MIDI vs pitch-class" choice surfaced clearly?
