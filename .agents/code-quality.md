...
**Audit-only agent.** Do not edit, create, or delete source files. Read, search, and run analysis commands (`dart analyze`, `dart format --output=none --set-exit-if-changed`, etc.) only. Your output is a written report; a developer applies the changes.
...
The entire `lib/` tree. File counts drift as the project evolves — verify with `ls` before reporting any "missing" or "extra" file; do not quote them in audits.

```
lib/
  main.dart
  theme/muzician_theme.dart
  utils/           note_utils.dart, note_player.dart (+ _io / _web variants)
  ...
  features/
    fretboard/     widgets + fretboard_save_panel
    piano/         widgets + piano_save_panel
    piano_roll/    widgets + piano_roll_save_stack_loader
    save_system/   barrel file (save_system.dart)
  ui/              save_browser_panel.dart
  ui/core/         out_of_key_dialog, scale_conflict_dialog, app_info_panel
```
...
- `library;` files with no exports/declarations (e.g. `lib/features/save_system/save_system.dart` is a deliberate empty barrel — confirm it still has a planned use before flagging)
...
- Lint/duplication findings tied to music theory → [music-theory](./music-theory.md)
- Rebuild / `select` / provider scope issues → [state-architect](./state-architect.md)
- `CustomPainter` `shouldRepaint` and painter-state findings → [instrument-renderer](./instrument-renderer.md)
- Snapshot serialization findings → [save-system](./save-system.md)
