/// PianoRollScalePicker – root note + scale type selector that highlights
/// pitch-class rows across the Piano Roll grid.
library;
...
class PianoRollScalePicker extends ConsumerStatefulWidget {
  const PianoRollScalePicker({super.key});

  @override
  ConsumerState<PianoRollScalePicker> createState() =>
      _PianoRollScalePickerState();
}

class _PianoRollScalePickerState extends ConsumerState<PianoRollScalePicker> {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(pianoRollProvider.select((s) => s.highlightedNotes), (
      prev,
      next,
    ) {
  ...
  Future<void> _tryApplyScale(String root, String scaleName) async {
    ...
    final currentNotes = ref.read(pianoRollProvider).notes;
    final scaleSet = scaleNotes.toSet();
    final conflicts = currentNotes
        .map((n) => n.pitchClass)
        .toSet()
    ...
    if (confirmed == true) {
      ref.read(pianoRollProvider.notifier).removeNotesByPitchClass(conflicts);
      setState(() {
        _selectedRoot = root;
        _selectedScale = scaleName;
      });
