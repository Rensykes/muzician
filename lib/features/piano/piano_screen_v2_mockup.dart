...
class PianoScreenV2Mockup extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PianoScreenV2Mockup> createState() =>
      _PianoScreenV2MockupState();
}
            ...
            Expanded(
              child: GlassFrame(
                ...
                child: _VerticalPianoCanvas(
                  ...
                  // up, not C2/C3/C5.
                  selectedMidis: state.selectedKeys
                      .map((k) => k.midiNote)
                      .toSet(),
...
class _VerticalPianoCanvasState extends State<_VerticalPianoCanvas> {
  ...
  Widget build(BuildContext context) {
    ...
    final keys = widget.viewMode == PianoViewMode.exactFocus
        ? allKeys
              .where((k) => widget.selectedMidis.contains(k.midiNote))
              .toList()
        : allKeys;
    return LayoutBuilder(
      builder: (ctx, c) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          ...
          if (midC < 0) return;
          final off = (midC * rowH - c.maxHeight / 2 + rowH / 2).clamp(
            0.0,
            totalH - c.maxHeight,
          );
    ...
    return GestureDetector(
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          border: Border(
            left: BorderSide(
              color: selected ? MuzicianTheme.scaffoldBg : Colors.transparent,
              width: 3,
            ),
            bottom: BorderSide(
