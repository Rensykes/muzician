...
void main() {
  group('PianoRollNotifier.firstEmptyColumnTick', () {
    test('returns 0 when there are no notes', () {
      ...
      expect(c.read(pianoRollProvider.notifier).firstEmptyColumnTick(), 0);
    });
    ...
    test('skips an occupied first column (snap 1)', () {
      ...
      addTearDown(c.dispose);
      expect(c.read(pianoRollProvider.notifier).firstEmptyColumnTick(), 1);
    });
    ...
    test('lands on the next free snap column (snap 2)', () {
      ...
      expect(c.read(pianoRollProvider.notifier).firstEmptyColumnTick(), 2);
    });
